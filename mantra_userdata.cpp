...
bool C_UserData::save_userdata(void)
{
	...
	strcpy(filename,va(_vx,"users%c%s",PATH_SEP,username));
	fout = fopen(va(_vx,"%s.tmp",filename),"wt");
	if(!fout) return false;

	fputs("** Mantra Server User Data\n",fout);
	fputs(va(_vx,"password=%s\n",password),fout);
	fputs(va(_vx,"access=%d\n",access),fout);
	fputs(va(_vx,"timeout_override=%d\n",timeout_override),fout);

	fclose(fout);

	remove(va(_vx,"%s.ini",filename));
	rename(va(_vx,"%s.tmp",filename),va(_vx,"%s.ini",filename));
	return true;
}
...
bool C_UserData::load_userdata(){
	...
	fp=fopen(va(_vx,"users%c%s.ini",PATH_SEP,username),"rt");
	if(!fp) return false;
	while(1)
	{
