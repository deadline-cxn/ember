...
bool C_UserData::load_userdata() {
    ...
    while (1) {
        ...
        if (v.size() == 2) {
            if (dlcs_strcasecmp(v[0].c_str(), "password")) strcpy(password, v[1].c_str());
            if (dlcs_strcasecmp(v[0].c_str(), "access")) access = atoi(v[1].c_str());
            if (dlcs_strcasecmp(v[0].c_str(), "timeout_override")) timeout_override = atoi(v[1].c_str());
        }
    }
    fclose(fp);
    return true;
...
void C_UserData::leave_channel(const char *ch) { chat_channel.erase(remove(chat_channel.begin(), chat_channel.end(), ch), chat_channel.end()); }

void C_UserData::join_channel(const char *ch) { chat_channel.push_back(ch); }
