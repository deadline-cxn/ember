...
void con_setgamemode(const string &s)   { pLog->_Add("setgamemode %s",(char *)s.c_str());  pClientData->GAME_MODE = (MODE) con_getint(s.c_str()); }
void con_guirender(const string &s) {
                                            ...
                                            {
                                                pGUI->get_stump(name)->set_caption(temp);
...
void con_guictrlrender(const string &s)                                        {
                                            ...
                                            {
                                                pLog->_Add("ADD CONTROL TO STUMP: %s",pGUI->get_stump(pstump)->stname);
                                                ...
                                                if(tctrl)
                                                {
                                                    ...
                                                    pLog->_Add("FOUND CTRL %s IN STUMP %s",tctrl->name,pGUI->get_stump(pstump)->stname);
                                                    tctrl->bEditing=true;
                                                }
                                            }
                                        }
void con_guiedit(const string &s) {
    ...
    if(pGUI->get_stump((char *) s.c_str()))
    {
        pGUI->get_stump((char *) s.c_str())->bEditing=true;
        pLog->_Add("con_guiedit: loaded %s",pGUI->getdata("name"));
    }
}
void con_guigo(const string &s)         { pGUI->parse((char *)s.c_str()); }
void con_guiclose(const string &s)      { if(pGUI->get_stump((char *)s.c_str())) pGUI->get_stump((char *)s.c_str())->bDeleteMe=true; }
void con_guistore(const string &s)      { pGUI->store(); }
...
void con_cvar(const string &s){
    ...
    if(!strlen(s.c_str()))
    {
        ...
        {
            pGUI->pConsole->get_cvar( (char *)(*pGUI->pConsole->svm_i).first.c_str(),temp);

             pLog->_Add("[%s]=[%s]",
                (*pGUI->pConsole->svm_i).first.c_str(),
                temp);
    ...
    if(vs.size()>1)
    {
        ...
        pGUI->pConsole->set_cvar(name,(char *)vs[1].c_str());
    }
    else strcpy(name,s.c_str());

    if(pGUI->pConsole->varmap.find(name)!=pGUI->pConsole->varmap.end())
    {
        pGUI->pConsole->get_cvar(name,temp);
...
void C_EM_CONSOLE::_GlobalFunctions(){
    ...
    funcmap["guicallfile"]  = con_guicallfile;
    funcmap["guistore"]     = con_guistore;
    funcmap["guiclose"]     = con_guiclose;
    funcmap["guirender"]    = con_guirender;
    funcmap["guictrlrender"]= con_guictrlrender;
    funcmap["guiedit"]      = con_guiedit;
...
void MainGameLoop(void){ // **  Main Game Loop
    ...
    //pGUI->WriteText(15,pClientData->ScreenHeight-64,va("MOUSE POS X[%d] Y[%d]",pGUI->pMouse->ix,pGUI->pMouse->iy),0); //pGFX->pCamera->xpos,pGFX->pCamera->ypos,pGFX->pCamera->zpos),0);
    //pGUI->WriteText(15,pClientData->ScreenHeight-48,va("CAM   POS X[%f] Y[%f] Z[%f]",pGFX->pCamera->xpos,pGFX->pCamera->ypos,pGFX->pCamera->zpos),0);
    //pGUI->WriteText(15,pClientData->ScreenHeight-32,va("CAM   ROT X[%f] Y[%f] Z[0.0] ANGLE[%f]",pGFX->pCamera->xrot,pGFX->pCamera->yrot,pGFX->pCamera->angle),0);
...
bool DoGameMode(void){
    ...
    case MAIN_MENU:
        ...
        //pGUI->call_file("test.gui");
        //pGUI->call("console.gui");
        //pGUI->store();
...
/*      if(!pModel)
        {
            pModel = new CGLModel(pLog);
            pModel->Load( "models/0000/tris.md2", "models/0000/skin.bmp" );
            pModel->Locate(0,0,0);
        }  */
...
long Ping(void){ // ** Ping the server
...
void GetFile(char *filename) { // ** Get a file from server
    CPacket SendData(NET_DATAGRAMSIZE);
	if(!pFMGS) return;
	SendData.Reset();
...
void DoNetwork(void) { // ** Network Loop
