...
CServer::CServer(int iPort, CLog *pInLog)
{
    initSocket();
    ...
    // pLog->_Add("Listening on port [%d]", iGetRemotePort());
...
/*	row=db->select("system id 1");
	...
	while(t)
	{
		pLog->_Add("%s=%s",t->name,t->value);
		t=t->next_var;
	}
	row=db->select("users name seth");
	pLog->_Add("seth is %s %s ( age = %s )",row->name,row->value,db->parse(row,"age"));
    pLog->_Add("%s",db->parse(row,"email"));
	db->row_update(db->select("users name seth"),"age","19");
	pLog->_Add("%s is %s %s ( age = %s )",
		db->parse(row,"name"),
		row->name,
		row->value,
		db->parse(row,"age")
    ...
    if(pMap==NULL)
    {
        pLog->_Add("Low memory can't create pMap!");
        bQuit=1;
        ShutDown();
    }
    ...
    if(pMapBuffer==NULL)
    {
        pLog->_Add("Low memory can't create pMapBuffer!");
        bQuit=1;
        ShutDown();
    }
    if(!pMapBuffer->Initialize(GMP_MAPSIZE*3,GMP_MAPSIZE*3,0,5))
    {
        pLog->_Add("Low memory can't initialize BigMap data!");
        bQuit=1;
        ShutDown();
    }
    pFragMap = new CEmMap;
    if(pFragMap==NULL)
    {
        pLog->_Add("Low memory can't create pFragMap!");
        bQuit=1;
        ShutDown();
    }
    ...
    for(i=0;i<10;i++)
    {
        if(!pFragMap)
        {
            pLog->_Add("Low memory can't create pFragMap sectors!");
            bQuit=1;
            ShutDown();
        }
...
void CServer::Accept(void)
{
    ...
    switch(cType)
    {
        case CTL_CONNECT:
            ...
            if(strcmp(Recv.pRead(),"EMBER"))
            {
                pLog->_Add("Fail attempt, not EMBER packet");
...
void CServer::Msg(char *name,char *msg)
{
	...
	while(pClient)
	{
		if(dlcs_strcasecmp(pClient->username,name))
		{
